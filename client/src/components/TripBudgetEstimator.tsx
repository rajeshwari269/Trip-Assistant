...
const TripBudgetEstimator: React.FC = () => {
  ...
  useEffect(() => {
    const fetchExchangeRates = async () => {
      try {
        setIsLoading(true);
        const response = await fetch(`https://v6.exchangerate-api.com/v6/${EXCHANGE_API_KEY}/latest/INR`);
  ...
  const handleRemoveExpense = (id: string) => {
    setExtraExpenses(expenses => expenses.filter(expense => expense.id !== id));
  };
  ...
  return (
    <div className={darkMode ? "p-6 max-w-2xl mx-auto bg-slate-600 border text-white rounded mt-5" : "p-6 max-w-2xl mx-auto bg-white rounded shadow mt-5"}>
        ...
        <div>
          ...
          <div className="relative">
            <input
              ...
              min="0"
              value={budgetPerDay}
              onChange={(e) => setBudgetPerDay(Math.max(0, parseInt(e.target.value)))}
              className="w-full p-3 border rounded mb-4 text-black bg-white pl-8 ps-4"
            />
          ...
          <div key={expense.id} className="flex gap-4 mb-4">
            ...
            <div className="relative w-1/3">
              <input
                ...
                onChange={(e) => handleExpenseChange(expense.id, 'amount', Math.max(0, parseFloat(e.target.value)))}
      ...
      {total !== null && (
        <div className="mt-6 p-4 bg-blue-50 rounded-lg border border-blue-200">
          <h4 className="text-lg font-semibold text-blue-900 mb-2">Budget Breakdown:</h4>
          <div className="space-y-2 text-blue-800">
            <p>Daily Budget: {formatCurrency(budgetPerDay)} × {days} days</p>
